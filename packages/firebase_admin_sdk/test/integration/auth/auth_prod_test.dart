...
void main() {
  group('setCustomUserClaims (Production)', () {
    test('clears custom claims when null is passed', () {
  ...
  group('Session Cookies (Production)', () {
    ...
    test('creates and verifies a valid session cookie', () {
      // Remove emulator env var from the zone environment
      final prodEnv = Map<String, String>.from(Platform.environment);
      prodEnv.remove(Environment.firebaseAuthEmulatorHost);
    ...
    // in runZoned to ensure the zone environment (without emulator) stays active.
    test('creates a revocable session cookie', () {
      // Remove emulator env var from the zone environment
      final prodEnv = Map<String, String>.from(Platform.environment);
      prodEnv.remove(Environment.firebaseAuthEmulatorHost);
      ...
      return runZoned(() async {
        ...
        try {
          final user = await testAuth.createUser(CreateRequest(uid: _uid.v4()));
          ...
          await expectLater(
            () =>
                testAuth.verifySessionCookie(sessionCookie, checkRevoked: true),
            throwsA(
    ...
    // and are not available in the Auth Emulator. This test wraps the test body
    // in runZoned to ensure the zone environment (without emulator) stays active.
    test('fails when ID token is revoked', () {
      // Remove emulator env var from the zone environment
      final prodEnv = Map<String, String>.from(Platform.environment);
      prodEnv.remove(Environment.firebaseAuthEmulatorHost);
    ...
    test('verifySessionCookie rejects invalid session cookie', () {
      // Remove emulator env var from the zone environment
      final prodEnv = Map<String, String>.from(Platform.environment);
      prodEnv.remove(Environment.firebaseAuthEmulatorHost);
  ...
  group('getUsers (Production)', () {
    test('gets multiple users by different identifiers', () {
      // Remove emulator env var from the zone environment
      final prodEnv = Map<String, String>.from(Platform.environment);
      prodEnv.remove(Environment.firebaseAuthEmulatorHost);
  ...
  group('createProviderConfig (Production)', () {
    ...
    // and are not available in the Auth Emulator. This test wraps the test body
    // in runZoned to ensure the zone environment (without emulator) stays active.
    test('creates OIDC provider config successfully', () {
      // Remove emulator env var from the zone environment
      final prodEnv = Map<String, String>.from(Platform.environment);
      prodEnv.remove(Environment.firebaseAuthEmulatorHost);
      ...
      return runZoned(() async {
        ...
        try {
          ...
          final createdConfig = await testAuth.createProviderConfig(oidcConfig);
    ...
    // and are not available in the Auth Emulator. This test wraps the test body
    // in runZoned to ensure the zone environment (without emulator) stays active.
    test('creates SAML provider config successfully', () {
      // Remove emulator env var from the zone environment
      final prodEnv = Map<String, String>.from(Platform.environment);
      prodEnv.remove(Environment.firebaseAuthEmulatorHost);
      ...
      return runZoned(() async {
        ...
        try {
          ...
          final createdConfig = await testAuth.createProviderConfig(samlConfig);
  ...
  group('getProviderConfig (Production)', () {
    test('returns the config for an existing OIDC provider', () {
      final prodEnv = Map<String, String>.from(Platform.environment);
      prodEnv.remove(Environment.firebaseAuthEmulatorHost);
      ...
      return runZoned(() async {
        ...
        final app = FirebaseApp.initializeApp(name: appName);
        final testAuth = Auth.internal(app);
        final providerId = 'oidc.get-${DateTime.now().millisecondsSinceEpoch}';
    ...
    test('throws FirebaseAuthAdminException for a non-existent provider', () {
      final prodEnv = Map<String, String>.from(Platform.environment);
      prodEnv.remove(Environment.firebaseAuthEmulatorHost);
  ...
  group('updateProviderConfig (Production)', () {
    test('updates an OIDC provider config', () {
      final prodEnv = Map<String, String>.from(Platform.environment);
      prodEnv.remove(Environment.firebaseAuthEmulatorHost);
    ...
    test('updates a SAML provider config', () {
      final prodEnv = Map<String, String>.from(Platform.environment);
      prodEnv.remove(Environment.firebaseAuthEmulatorHost);
  ...
  group('listProviderConfigs (Production)', () {
    test('lists OIDC provider configs including one just created', () {
      final prodEnv = Map<String, String>.from(Platform.environment);
      prodEnv.remove(Environment.firebaseAuthEmulatorHost);
      ...
      return runZoned(() async {
        ...
        final app = FirebaseApp.initializeApp(name: appName);
        final testAuth = Auth.internal(app);
        final providerId = 'oidc.list-${DateTime.now().millisecondsSinceEpoch}';
    ...
    test('lists SAML provider configs including one just created', () {
      final prodEnv = Map<String, String>.from(Platform.environment);
      prodEnv.remove(Environment.firebaseAuthEmulatorHost);
      ...
      return runZoned(() async {
        ...
        final app = FirebaseApp.initializeApp(name: appName);
        final testAuth = Auth.internal(app);
        final providerId = 'saml.list-${DateTime.now().millisecondsSinceEpoch}';
    ...
    test('supports pagination with maxResults and pageToken', () {
      final prodEnv = Map<String, String>.from(Platform.environment);
      prodEnv.remove(Environment.firebaseAuthEmulatorHost);
